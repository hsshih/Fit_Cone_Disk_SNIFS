...
##def read_bestparms(objname=''):
##
##    parmfiles = glob.glob(objname+'*bestparms*')
##    
##    for bestparfile in parmfiles:
##        try:
##            parfile = open(bestparfile, 'rb')
##            parlist0 = pickle.load(parfile)
##            parfile.close()
##            bestchi0 = [o.chi2 for o in parlist0]
##            trialnums0 = [o.trialnum for o in parlist0]
##
##            sortchi0 = sorted(range(len(bestchi0)), key=lambda k: bestchi0[k])
##            idx = sortchi0[0:5]
##            #print "Object is: ", parlist0[0].objname
##            print "Best Chi squared for", bestparfile, " is: ", [bestchi0[i] for i in idx]
##            print "Best trial number for", bestparfile, " is: ", [trialnums0[i] for i in idx]
##            print " "
##        
##        except IOError as e:
...
def mk_initial_input():
    ...
    inilist = []
# parnames1 = ['iangle', 'vc', 'pa', 'xoff', 'yoff', 'veloff'] - disk
# parnames2 = ['oangle1','iangle', 'pa', 'k1', 'xoff', 'yoff', 'veloff'] - bicone
#**** Somehow x offsets are reversed for the disk.....
...
## FBQSJ122539.5+245836 - can't be described by disk-bicone model
...
# Perform mpfit on slightly randomly perturbed intitial guesses
        ...
        for i in xrange(nloop):
            ...
            ## Fraction of disk component in the model
            ##fracdisk = np.random.uniform(low=0.0, high=1.0, size=1)
            ##fracdisk=1.

            loopnum=i+startnum
            
       #     pngname = objname + '_mpfit_backonly' + `loopnum` + '.png'
       #     mpfit_diskcone(velfitsname, objname=objname, piguess1=p1, piguess2=p2, highcut=highcut,\
       #                        lowcut=lowcut, loopnum=loopnum, pngname=pngname, frontonly=0, backonly=1, cone_oangle2=20)
       #     bestparfile = objname + '_mpfit_bestparms_backonly'

        #    pngname = objname + '_mpfit_frontonly' + `loopnum` + '.png'
        #    mpfit_diskcone(velfitsname, objname=objname, piguess1=p1, piguess2=p2, highcut=highcut,\
        #                       lowcut=lowcut, loopnum=loopnum, pngname=pngname, frontonly=1, backonly=0, cone_oangle2=20)
        #    bestparfile = objname + '_mpfit_bestparms_frontonly'
        ...
        #    pngname = objname + '_mpfit_half_solidcone_frontonly' + `loopnum` + '.png'
        #    mpfit_diskcone(velfitsname, objname=objname, piguess1=p1, piguess2=p2, fracdisk=0.5, highcut=highcut,\
        #                   lowcut=lowcut, loopnum=loopnum, pngname=pngname, frontonly=1, backonly=0)
        #    bestparfile = objname + '_mpfit_half_bestparms_solidcone_frontonly'
...
def proj_bicone(radius=30, oangle1=30., oangle2=15., pa=0., iangle=0., k1=1., k2=0., vmax=2000.,
 ...
 #   cospa = math.cos(pa)   # Will rotate the projected image later
 #   sinpa = math.sin(pa)
    cospa = 1.
    sinpa = 0.
...
#For fitting purpose need to make y,z dimension the same, cube will be summed in x dimension so x size just need to be big enough
 ...
 # If frontonly set to one only take average of the first n=depth non-zero pixels for every projected velocity data point
   ...
   # plt.colorbar()
   # plt.savefig("projbicone_i60_pa0_solid_backonly.png")
...
#    del yarr
#    del zarr
 #   print 4, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    return conevel

 #   print xarr[0], yarr[0], alos[0]*180./pi, conevel[0]
...
def rotdisk_solid(iangle=45.,vc=100., pa=0, radius=5, centeroffset=[0,0], veloffset=0.):
    ...
    #plt.clf()
    #plt.scatter(xcoords, ycoords, c=vels, cmap='spring')
 ...
 #   print iangle, pa, radius, centeroffset
 ...
 #   plt.imshow(grid, interpolation='none', origin='lower')
 #   plt.colorbar()
 #   plt.savefig("disk_iangle45_solid.png")
...
def rotring_solid(iangle=0., vc=100., radius=5, maxrad=10):
 ...
 #   psi = np.arctan(np.float_(ringy)/np.float_(ringx))
 #   posx = ringx > 0
 #   negx = ringx <= 0
 #   posy = ringy > 0
 #   negy = ringy <= 0
 ...
 #   plt.clf()
 #   plt.scatter(ringx, ringy, c=ringvel, cmap='spring')
...
def mkmask(fitsdata, lowcut=-300, highcut=300):
 ...
 #   plt.clf()
 #   plt.scatter(datacoord[0], datacoord[1])
...
#    plt.clf()
#    plt.scatter(goodx[cluster1[0]], goody[cluster1[0]])
#    plt.savefig("coords2.png")
...
#Bicone Component
    ...
    #pngname = objname + '_mcfit_cone' + `loopnum` + '.png'
    #mcfit(velfitsname, parlimits1, npar1, parlimits2, npar2, nsample, highcut=-50, lowcut=-300, pngname=pngname, fracdisk=0, objname=objname, trialnum=loopnum)
    #pngname = objname + '_mcfit' + `loopnum` + '.png'
    #mcfit(velfitsname, parlimits1, npar1, parlimits2, npar2, nsample, highcut=highcut, lowcut=lowcut, pngname=pngname, objname=objname, trialnum=loopnum)
...
#    f2 = open('Bestfit_conedisk', 'a')
#    f2.write(pngname)
#    f2.write(str(bestparm1) + '\n')
#    f2.write(str(bestparm2) + '\n')
#    f2.write(str(bestfracdisk) + '\n')
#    f2.write(str(lowestchi2) + '\n')
#    f2.close()
...
#    axes[0].imshow(fitmodel, interpolation='none', origin='lower', vmin=-500, vmax=500)
...
#    f2.close()
    ...
    fitmodel1 = rotdisk(iangle=bestparm1[0], vc=bestparm1[1], pa=bestparm1[2], radius=7, centeroffset=[bestparm1[3],bestparm1[4]], veloffset=bestparm1[5])
